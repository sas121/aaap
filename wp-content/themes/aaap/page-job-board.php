...
//    AND  (($wpdb->posts.post_type='job_board' AND $wpdb->postmeta.meta_key = 'home_index') OR ($wpdb->posts.post_type = 'post' AND $wpdb->terms.name='News')) 
    ...
    <div class="page_content">
        ...
        <div class="page_main_content policy">
            ...
            <?php foreach ($pageposts as $pagepost): ?>
                <?php
                $linkType = get_post_meta($pagepost->ID, '_job_board_link_type', true);
                $linkUrl = get_post_meta($pagepost->ID, '_job_board_link_url', true);
                $pdfURL = get_post_meta($pagepost->ID, '_job_board_pdf_url', true);
                $location = get_post_meta($pagepost->ID, '_job_board_location', true);
                ...
                if ($pagepost->name == 'News') {
                    ...
                    $image = get_post_meta($pagepost->ID, '_job_board_link_image', true);
                    //$description = get_post_meta($pagepost->ID, '_job_board_description', true);
                    $description = $pagepost->post_excerpt;
                ...
                <div class="people_info">
                    ...
                    <?php endif; ?>
                    <div class="info">
                        <h3><a href="<?php echo $pagepost->guid ?>"><?php echo $pagepost->post_title; ?></a></h3>
                        <div class="location"><?php echo $location; ?></div>
                        <div class="title-job-board date-no-uppercase"><?php echo $date; ?></div>
                        <div class="description">
                            <?php echo $description; ?>
							<?php if( ! empty($linkType) ): ?>
                            <a <?php echo $target; ?> href="<?php echo $link; ?>" class="box_link">Job Description</a>
							<?php endif; ?>
                        </div>
                    </div>
                    <div class="clear"></div>
